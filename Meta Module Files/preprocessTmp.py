...
def create_inputs(splits, output):
    ...
    for split in splits:
        ...
        for idx, entry in enumerate(data):
            ...
            if idx == 0:
                print(entry)
                print(imageId, question, programs, questionId)
            ...
            try:
                ...
                graph = {}
                skipped += 1
                #sys.stdout.write("Skipped {} \r".format(skipped))
                
            rounds, tmp, connection, inputs, execution_buffer, bounding_box, tmp_connection = ([] for _ in range(7))
            depth = {}
            cur_depth = 0
            ...
            for i, program in enumerate(programs):
                ...
                if graph:             
                    ...
                    try:
                        ...
                        print("EXCEPTION", "@", entry[1], "@", imageId, "@", program)
                        ...
                        if isinstance(returns, list):
                            if result[0] == '[' and result[-1] == ']':
                                if result == '[]':
                                    ...
                                    bounding_box.append((graph['objects'][result]['x'], graph['objects'][result]['y'],
                                                         graph['objects'][result]['w'], graph['objects'][result]['h']))
                            else:
                                if returns:
                                    bounding_box.append((graph['objects'][returns[0]]['x'], graph['objects'][returns[0]]['y'],
                                                         graph['objects'][returns[0]]['w'], graph['objects'][returns[0]]['h']))
                                else:
                                    bounding_box.append((-1, -1, -1, -1))
                            execution_buffer.append(returns)
                        elif isinstance(returns, tuple):
                            if returns[0]:
                                bounding_box.append((graph['objects'][returns[1]]['x'], graph['objects'][returns[1]]['y'],
                                                     graph['objects'][returns[1]]['w'], graph['objects'][returns[1]]['h']))
                            else:
                                bounding_box.append((-1, -1, -1, -1))
                            execution_buffer.append(returns[0])
                        else:
            ...
            if idx == 0:
                print([entry[0], entry[1], bounding_box, inputs, connection, questionId, entry[-1]])
                
            results.append([entry[0], entry[1], bounding_box, inputs, connection, questionId, entry[-1]])
