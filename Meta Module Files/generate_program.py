...
def parse_opt():
    ...
    parser.add_argument('--do_train_calibrated', default=False, action="store_true", help="whether to train or test the model")
    ...
    parser.add_argument('--max_samples', default=None, type=int, help="The hidden size of the state")
    parser.add_argument('--name', default="", type=str, help="The hidden size of the state")
...
class GQA(Dataset):
    ...
    def __init__(self, vocab, split):
        ...
        if split == 'train':
            if args.max_samples is not None:
                ...
                with open('{}/train_pairs.json'.format(args.output)) as f:
                    self.data = json.load(f)          
        elif split == 'train_calibrated_fully':
            with open('{}/train_pairs.json'.format(args.output)) as f:
                    self.data = json.load(f)  
        ...
        if args.max_samples is not None:
            self.samples = random.sample(range(len(self.data)), args.max_samples)

    def __getitem__(self, index):
        if self.samples is not None:
            index = self.samples[index % len(self.samples)]
...
def train(option):
    ...
    #model = TransformerDecoder(len(vocab), args.hidden_dim, 6, args.hidden_dim, 8)
    ...
    if option == 'train':
        ...
        for epoch in range(args.max_epoch):
            for idx, batch in enumerate(data_loader):
                ...
                if idx % 100 == 0:
                    print("step {}, loss = {}, time spent = {}".format(idx, loss.item(), time.time() - start_time))
                    start_time = time.time()

                epoch_loss += loss.item()

            torch.save(model.state_dict(), 'models/seq2seq{}_ep{}.pt'.format(args.name, epoch))

    elif option in ['trainval_unbiased', 'testdev', 'submission','train_calibrated_fully']:
        ...
        if not args.debug:
            if option == 'submission':
                with open('../processed/questions/needed_submission_programs.json'.format(option), 'w') as f:
                    json.dump(generated, f, indent=2)
            elif option == 'testdev':
                with open('../processed/questions/testdev_pred{}_programs.json'.format(args.version), 'w') as f:
                    json.dump(generated, f, indent=2)
            else:
                with open('../processed/questions/{}_programs{}.json'.format(option, args.version), 'w') as f:
                    json.dump(generated, f, indent=2)
        else:
...
if args.do_preprocess:
    create_pairs('../processed/questions/trainval_all_programs.json', 'train')
    print("finished creating the pairs")
    #create_vocab('{}/train_pairs.json'.format(args.output))
    create_pairs('../processed/questions/trainval_balanced_programs.json', 'train_balanced')
    create_pairs('../processed/questions/testdev_balanced_programs.json', 'testdev')
    create_pairs('../processed/questions/needed_submission_questions.json', 'submission')
elif args.do_train:
    train('train')
elif args.do_train_calibrated:
    train('train_calibrated_fully')
